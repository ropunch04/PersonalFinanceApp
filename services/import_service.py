...
def _source_hash(provider: str, transaction_at: str, amount: float, merchant_raw: str, row_num: int) -> str:
...
def _get_category_id(conn, merchant_raw: str) -> int:
    ...
    if row:
        return row["id"]
    other = conn.execute(
        "SELECT id FROM categories WHERE LOWER(name) = 'other'"
    ).fetchone()
    ...
    for row_num, row in enumerate(reader, start=2):
        try:
            ...
            transactions.append({
                ...
                "source_hash":    _source_hash("capitalone", transaction_at, amount, description, row_num),
            })
    ...
    for row_num, row in enumerate(reader, start=4):
        try:
            ...
            if amount_raw.startswith("+"):
                ...
                errors.append({"row": row_num, "reason": f"Unexpected amount format: {amount_raw!r}", "data": dict(row)})
                continue
            ...
            transactions.append({
                ...
                "transaction_at": transaction_at,
                "source_hash":    _source_hash("venmo", transaction_at, amount, merchant_raw, row_num),
            })
